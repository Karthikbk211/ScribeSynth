...
class CVLDataset(Dataset):
    def __init__(self, image_path, style_path, freq_path, type,
        ...
        self.freq_path = os.path.join(freq_path, type)
    ...
    def get_style_ref(self, wr_id):
        ...
        style_images = [img / 255.0 for img in style_images]
        new_style = np.ones([2, height, max_w], dtype=np.float32)
        new_style[0, :, :style_images[0].shape[1]] = style_images[0]
        new_style[1, :, :style_images[1].shape[1]] = style_images[1]

        freq_images = [img / 255.0 for img in freq_images]
        new_freq = np.zeros([2, height, max_w], dtype=np.float32)
        new_freq[0, :, :freq_images[0].shape[1]] = freq_images[0]
        new_freq[1, :, :freq_images[1].shape[1]] = freq_images[1]

        return new_style, new_freq
    ...
    def label_padding(self, labels, max_len):
        ...
        num = max_len - len(ll)
        if num != 0:
            ll.extend([self.tokens["PAD_TOKEN"]] * num)
        return ll
    ...
    def __getitem__(self, idx):
        ...
        style_ref, freq_ref = self.get_style_ref(wr_id)
        style_ref = torch.from_numpy(style_ref).to(torch.float32)
        freq_ref = torch.from_numpy(freq_ref).to(torch.float32)
    ...
    def collate_fn_(self, batch):
        ...
        max_s_width = min(max(s_width), self.style_len)

        imgs = torch.ones([len(batch), batch[0]['img'].shape[0],
                           batch[0]['img'].shape[1], max(width)], dtype=torch.float32)
        content_ref = torch.zeros([len(batch), max(c_width), 16, 16], dtype=torch.float32)
        style_ref = torch.ones([len(batch), batch[0]['style'].shape[0],
                                batch[0]['style'].shape[1], max_s_width], dtype=torch.float32)
        freq_ref = torch.zeros([len(batch), batch[0]['freq'].shape[0],
                                batch[0]['freq'].shape[1], max_s_width], dtype=torch.float32)
        ...
        for idx, item in enumerate(batch):
            ...
            target[idx, :len(transcr[idx])] = torch.Tensor(
                [self.letter2index[t] for t in transcr[idx]])
        ...
        return {
            ...
            'wid': wid, 'freq': freq_ref, 'target': target,
            'target_lengths': target_lengths, 'image_name': image_name
        }
...
class Random_StyleCVLDataset(CVLDataset):
    ...
    def get_style_ref(self, wr_id):
        ...
        return style_image, freq_image

    def __getitem__(self, _):
        batch = []
        for idx in self.author_id:
            style_ref, freq_ref = self.get_style_ref(idx)
            style_ref = torch.from_numpy(style_ref).unsqueeze(0).to(torch.float32)
            freq_ref = torch.from_numpy(freq_ref).unsqueeze(0).to(torch.float32)
            batch.append({'style': style_ref, 'freq': freq_ref, 'wid': idx})
